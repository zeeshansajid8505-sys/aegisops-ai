...
export interface AlertCorrelationInput {
  ...
  /**
   * Set of 1-hop connected service IDs (both services this service depends on and services depending on this service)
   */
...
/**
 * Calculates temporal proximity bonus based on elapsed seconds between alert and candidate's last signal.
 */
...
/**
 * Scores a single active incident candidate against an incoming firing alert.
 */
export function scoreIncidentCandidate(
  ...
  // If no topological or team connection at all, do NOT correlate on pure time proximity alone
  if (topologyScore === 0) {
    return null;
  }
...
/**
 * Evaluates a list of candidate incidents and selects the single best matching incident
 * using deterministic tie-breaking rules:
 * 1. Highest total correlation score
 * 2. Most recent lastSignalAt
 * 3. Highest incident severity
 * 4. Stable incident ID comparison (localeCompare)
 */
